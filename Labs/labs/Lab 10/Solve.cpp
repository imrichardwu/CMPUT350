...
void best_response_to_col([[maybe_unused]] const Matrix &A, [[maybe_unused]] const Vector &strat2) {
    assert(A.size() > 0);
    int N1 = A.size();          // Number of rows (p1's moves)
    int N2 = A[0].size();       // Number of columns (p2's moves)

    assert((int)strat2.size() == N2);

    double max_val = -1e308;    // Initialize to a very small number
    int max_ind = -1;           // Index of the best response row

    // Iterate over each row to find the one with the maximum expected payoff
    for (int i = 0; i < N1; ++i) {
        double sum = 0.0;
    ...
    // Construct the best response strategy (pure strategy)
    Vector best_response(N1, 0.0);
    if (max_ind != -1) {
        best_response[max_ind] = 1.0;
    }

    // Output the best response strategy
    cout << "best response to col strategy:";
    for (const auto &prob : best_response) {
        cout << " " << prob;
    }
    cout << endl;

    // Output the value for p1
    cout << "value for p1: " << max_val << endl;
}
