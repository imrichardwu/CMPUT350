/*
  ...
  Solve a zero-sum matrix game read from stdin by linear programming
  ...
  M doubles (col player strategy, for options -c and -e)
  ...
  -s:
    ...
    - game value for p1
    - optimal strategies for p1 and p2
    - also creates files p?.lp and p?.res that contain LPs and their solutions
  ...
  -c:
    ...
    - best response strategy to given col strategy
    - game value for p1
...
int main(int argc, const char **argv) {
    if (argc != 2) {
        cerr << "*** call: " << argv[0] << " (-s | -r | -c | -e) " << endl
             ...
             << " -c : compute best response to col strategy" << endl
