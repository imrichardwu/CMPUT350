    ...
    */
    void set(const string &s) {
        ...
        */
        ...
        char c;
        filled = 0;
        to_move = MAX;  // Initialize MAX to move

        for (int y = 0; y < N; ++y) {
            for (int x = 0; x < N; ++x) {
                if (!is.get(c)) {
                    throw InputException("Input string too short");
                }
                ...
                // Convert character to board state
                if (c == get_disp(MAX)) {
                    sq[y][x] = MAX;
                    filled++;
                    ...
                    throw InputException("Invalid character in input string");
                }
            }
        }
    }
