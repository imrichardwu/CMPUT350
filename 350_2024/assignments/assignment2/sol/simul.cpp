...
public:
    GfxWorld(double w, double h, int seed) : World(w, h, seed) {}
  ...
  glEnd();
    ...
    for (size_t i = 0; i < world->units.size(); ++i) {
        ...
        for (size_t j = 0; j <= 16; ++j) {
            ...
            glVertex2f(u.pos.x + cos(angle) * u.radius, u.pos.y + sin(angle) * u.radius);
...
int main(int argc, char *argv[]) {
    if (argc != 1 && argc != 11) {
        cerr << "usage: " << argv[0] << " width height delay seed marines tanks redpol bluepol bounce gfx" << endl;
    ...
    // get unit sizes - a bit awkward, could be data member
