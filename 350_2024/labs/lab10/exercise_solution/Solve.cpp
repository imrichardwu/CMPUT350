...
// solve matrix game, print MiniMax strategies, and value for p1
...
void solve(Matrix &A) {
...

    {
        /*
          ...
          (for the exact syntax run ./solve -s < test2x4.mg and look at p1.lp)
        */
        ...
        for (int j = 0; j < N2; j++) {
            fprintf(fp, "Z+%.10f ", min_A);
            for (int i = 0; i < N1; i++) {
                fprintf(fp, " - %.10f x%d", A[i][j], i);
            }
            fprintf(fp, " <= 0;\n");
        }
...

    {
        /*
          ...
          (for the exact syntax run ./solve -s < test2x4.mg and look at p2.lp)
        */
...
// compute best response to row strategy and value for p1
...
// compute best response to col strategy and value for p1
...
void best_response_to_col(const Matrix &A, const Vector &strat2) {
    ...
    cout << endl << "value for p1: " << max_val << endl;
}

// compute expected value of game for given row and col strategies for the row player
