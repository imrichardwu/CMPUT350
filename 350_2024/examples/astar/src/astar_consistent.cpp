...
std::pair<bool, std::vector<int>> a_star(std::unique_ptr<BaseState> puzzle) {
    ...
    while (!open.empty()) {
        ...
        for (const auto &action : n->state->child_actions()) {
            ...
            if (closed.find(child_node.get()) == closed.end()) {
                open.push(child_node.get());
...
// /usr/bin/time --verbose ./src/astar_consistent sliding_puzzle 7,4,5,12,15,0,6,1,8,14,11,13,10,9,2,3
