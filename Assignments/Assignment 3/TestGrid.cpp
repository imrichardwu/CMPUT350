...
static int MouseToCoordX(int x) {
    return std::max(0, std::min(x * mymap->getWidth() / windowWidth, mymap->getWidth() - objectSize - 1));
}

static int MouseToCoordY(int y) {
    return std::max(0, std::min(y * mymap->getHeight() / windowHeight, mymap->getHeight() - objectSize - 1));
}
...
void OnDisplay() {
    ...
    if (mouseLeft && mymap->isConnected(objectSize, mouseX0, mouseY0, mouseX1, mouseY1)) {
        mymap->findShortestPath(objectSize, mouseX0, mouseY0, mouseX1, mouseY1, path);
    }

    static const float colors[][3] = {{0, 0.5f, 0}, {0, 0, 0.5f}, {0.25f, 0.25f, 0.25f}};
