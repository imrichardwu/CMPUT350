...
class World {
public:
    World(double w, double h, int seed) : width(w), height(h) {
        rng.seed(seed);
    }
    ...
    virtual void attack_hook(const Unit & /*attacker*/, const Unit & /*attacked*/) {}
    ...
    // clears vector first
    void enemies_within_attack_range(const Unit &u, std::vector<Unit *> &targets) const;
